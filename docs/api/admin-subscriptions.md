...
## `GET /admin/subscriptions/:id/payment-methods`

Lists the saved payment methods of the subscription's customer for the subscription's payment provider.
...
- the payment method must be a saved payment method of the subscription's customer for that provider
- allowed for `active`, `paused` and `past_due` subscriptions
- records a `subscription.payment_method_updated` activity-log event
...
- `400 invalid_data`
  Invalid body payload, unknown payment method, or payment method not owned by the customer.
- `404 not_found`
  Subscription does not exist.
- `409 conflict`
...
- The subscription detail payload includes `payment_provider_id` and a normalized `payment_method` summary. `payment_method` is `null` when no payment method is stored or when the stored one can no longer be resolved from the payment provider.
